...
# 只看配置不够：服务商条目可能配好了但服务没起（Windows 上常见——没装 Ollama）。
# 这里真探一次服务可达性（/api/version 返回 200 即视为可用），不可达就 SKIP，
# 免得把一个「环境缺失」误报成「功能失败」。
_reachable = False
try:
    import urllib.request
    from livetrans.sysmon import ollama_root
    _root = ollama_root(getattr(ollama, "base_url", None))
    with urllib.request.urlopen(f"{_root}/api/version", timeout=2.0) as _r:
        _reachable = _r.status == 200
except Exception:  # noqa: BLE001
    _reachable = False

if not _reachable:
    print("[SKIP] 本机 Ollama 服务不可达，跳过（端到端需要本地模型服务）")
    raise SystemExit(0)
...
log_path = Path(tempfile.gettempdir()) / "e2e-dialog.jsonl"
