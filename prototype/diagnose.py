"""内部音源诊断探针：monitor 捕获 -> VAD 切段 -> SenseVoice 转写，逐段打印。

用途：网页/应用播放声音时，验证"系统输出 -> monitor -> 识别"链路是否通畅。
没有字幕时先跑它：能转写出文本 = 链路 OK（问题在别处）；overall_rms≈0 = 
系统输出设备上根本没有声音（查音量/静音/播放设备选择）。

用法（prototype 目录下）：
  python3 diagnose.py [秒数=20] [语言=en|zh|auto]
示例：
  python3 diagnose.py 20 en      # 浏览器播放英文视频时运行，20 秒后出结论
"""
import queue
import sys
import time
...
def main() -> int:
    ...
    if not mon:
        print("未找到 monitor 源（确认桌面使用 PulseAudio/PipeWire）")
        return 1
    ...
    if rms < 0.002:
        print("=> monitor 上没有声音：检查播放器/系统音量与静音、"
              "声音是否输出了别的设备、网页是否真的在播放。")
        return 1
    if segs == 0:
        print("=> 有声音但 VAD 未切出语音段（音量太小或非语音内容）。")
        return 1
    print("=> 链路正常：monitor 能收到声音并识别出文本。")
    return 0
