...
def _para_text(p: _Para) -> str:
    ...
    if p.revised is None:
        return _join_parts([(t or "…") for t in p.dsts])
    parts = [p.revised]
    parts += [(p.dsts[i] or "…")
...
class TranslatorWorker(threading.Thread):
    ...
    def _para_for(self, ev: SegmentEvent, role: str | None) -> _Para:
        ...
        if p is not None and ev.gap_ms <= gap_thr and len(p.srcs) < MAX_PARA_SENTS:
            # 并入当前段落：原文追加，译文留待本句翻译
            p.srcs.append(ev.text)
            p.dsts.append(None)
            self.window.update_source(p.para_id, _join_parts(p.srcs))
            return p
            ...
            if p.pending_since_revise > 0 and len(p.dsts) >= 2:
                ...
                self._maybe_revise(p, tr, p.role or None,
                                   max(1, int(getattr(self.translator.cfg,
                                                      "revise_depth", 2))),
                                   force=True)
        para_id = f"para-{ev.source_key}-{ev.ts:.3f}-{self._seq}"
        if role:
            para_id += f"-{role}"
        p = _Para(para_id=para_id, ts=ev.ts, kind=ev.kind, label=ev.label,
                  app=ev.app, role=role or "", srcs=[ev.text], dsts=[None])
        self._paras[key] = p
        self.window.post(DisplayItem(
            kind=ev.kind, label=ev.label, app=ev.app,
            text=ev.text, translation=None, item_id=para_id,
            ts=ev.ts, asr_ms=ev.asr_ms, speaker=ev.speaker, role=role or ""))
        return p
...
def main(argv=None) -> int:
    # ---- 子模式分派（必须在 argparse 之前）----
    # PyInstaller 打包后 `python -m livetrans.overlay` 不可用，
    # 控制台改以 `LiveTrans.exe --overlay [--config ...]` 拉起字幕外挂。
    # 这里先把 --overlay 摘出来交给 overlay 自己的 parser，避免两边参数定义打架。
    raw = list(sys.argv[1:] if argv is None else argv)
    if "--overlay" in raw:
        i = raw.index("--overlay")
        rest = raw[:i] + raw[i + 1:]
        from .overlay import main as _overlay_main
        return _overlay_main(rest)

    ap = argparse.ArgumentParser(
        prog="livetrans", description="LiveTrans 实时语音翻译（Windows 桌面）")
    ...
    ap.add_argument("--overlay", action="store_true",
                    help="改跑字幕外挂（等价 -m livetrans.overlay；打包后用这个）")
    ...
    # launcher 拉起主程序时会用 merged_env() 传 env；但用户**直接跑 exe/模块**时
    ...
    def boot() -> None:
        try:
            ...
            if cfg.audio.monitor_enabled:
                window.set_status("连接系统音频源 ...")
                mon_src = cfg.audio.monitor_source or default_monitor_source()
                if mon_src:
                    _log(f"使用系统音频源: {mon_src}")
                    try:
                        # Windows：ParecCapture 内部走 WASAPI loopback（类名沿用 Linux 版）
                    ...
                    # 后备：PortAudio 枚举输入设备（Windows 上一般是虚拟声卡的 loopback）
                    mon = cfg.audio.monitor_device
                    if mon is None:
                        mon = find_monitor_device()
                    if mon is None:
                        _log("⚠ 未找到系统音频捕获设备，跳过内部音频"
                             "（确认已安装 soundcard：pip install soundcard，"
                             "且系统存在可用的播放设备）")
                        window.set_status("⚠ 未找到系统音频源，仅麦克风可用")
                    else:
                        try:
