...
实时语音翻译（Linux 桌面）：捕获**系统播放的声音**与**麦克风**，用本地语音识别转写，
...
依赖清单与说明见 [`prototype/requirements.txt`](prototype/requirements.txt)；控制台在启动外挂前会自检组件，
缺什么会直接给出对应的 `apt install` 命令。
...
packaging/                  Debian 打包脚本（build_deb.sh）+ 桌面项 + pip 依赖脚本
livetrans/
  main.py                   主程序入口：多路音频 → 识别 → 翻译 → 主字幕窗
  overlay.py                字幕外挂（GTK3 悬浮窗 + 控制面板 + 自带管线）
  ...
  capture.py                麦克风（PortAudio）与系统声音（parec）采集
...
- **字幕不出/很慢**：看 `prototype/run.log`（历史轮转在 `run.log.1…5`）。云端模型若开启了深度思考会明显变慢，控制台「测速」可对比；本地模型首次要加载（数秒~1 分钟）。
...
- 日志：控制台启动的程序输出写入 `prototype/run.log`，启动前轮转（保留 run.log.1…5）
- 会话数据：`prototype/sessions/*.jsonl`（含模型原始输出，便于复盘）
- 依赖自检：`python3 -c "from livetrans.deps import missing_pip_deps, missing_overlay_deps; print(missing_pip_deps(), missing_overlay_deps())"`
...
# 2) 发新版本：改版本号（唯一来源）→ 打 tag → 推送，CI 自动出两种 .deb
$EDITOR prototype/livetrans/__init__.py     # __version__ = "1.1.0"
git commit -am "bump 1.1.0" && git push
git tag v1.1.0 && git push origin v1.1.0    # ← 这条会触发 .github/workflows/release.yml
```

- 发版工作流会自动产出 **两个安装包**（普通包 + 完全离线包）与 `SHA256SUMS`，挂在 GitHub Release 上
- 提代码/PR 会跑 `.github/workflows/test.yml`（xvfb 下执行 `selfcheck` + 全部专项测试）
