...
> **✅ 状态更新（2026-09-18）**：A 类各项已完成 Linux 侧移植（分 6 个提交，
> 每步全量测试绿灯；B 类按"不移植"处理；A-5 经核实 Linux GTK 版外挂
> 无此缺陷，无需改动）。逐项说明见 `TECH_ROADMAP.md` 开发日志 2026-09-18 条目。
> C 类待办不变：Linux 侧自测通过后自行 bump 版本 + 打标签发版。
